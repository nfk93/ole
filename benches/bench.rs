...
pub fn bench_euclid_division(c: &mut Criterion) {
    ...
    let b: Vec<Fp> = (0..nb).map(|_| Fp::random(&mut rng)).collect();
    c.bench_function(
    ...
    c.bench_function("ole init", move |b_| {
        b_.iter(|| {
            ...
            let handle = std::thread::spawn(move || {
                ...
                let writer = BufWriter::new(sender);
                let mut sender_channel = Channel::new(reader, writer);
                let _olesender = OleSender::init(&mut sender_channel, &mut rng).unwrap();
            });
            let mut rng = rand::thread_rng();
            let reader = BufReader::new(receiver.try_clone().unwrap());
            let writer = BufWriter::new(receiver);
            let mut receiver_channel = Channel::new(reader, writer);
            let _olereceiver = OleReceiver::init(&mut receiver_channel, &mut rng).unwrap();
            handle.join().unwrap();
        })
    });
}
