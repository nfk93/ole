...
fn euclid_division(a: &[Fp], b: &[Fp]) -> (Vec<Fp>, Vec<Fp>) {
    let mut deg_r = a.len() - 1;
    let mut q = vec!(Fp::zero(); deg_r + 1);
    let mut r = a.to_vec();
...
fn naive_poly_mult(a: &[Fp], b: &[Fp]) -> Vec<Fp> {
    let mut prod = vec!(Fp::zero(); a.len() + b.len() - 1);
